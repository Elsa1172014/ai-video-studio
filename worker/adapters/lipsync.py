...
class LipSyncAdapter(VideoAdapter):
 async def generate(self,prompt:str,duration:int,image_url:str|None=None)->str:
  cmd=os.getenv("LIPSYNC_GENERATE_COMMAND")
  if not cmd:raise RuntimeError("Configure LIPSYNC_GENERATE_COMMAND for MuseTalk")
  ...
  if p.returncode or not out.exists():raise RuntimeError(err.decode()[-2000:] or "Lip-sync failed")
